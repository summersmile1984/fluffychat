...
class Client extends MatrixApi {
  ...
  /// RFC 8707 resource indicators for OIDC JWT access_tokens.
  /// Set by FluffyChat during login; used during token refresh to get JWT.
  List<String>? oidcResource;
  ...
  Future<void> refreshAccessToken({
    ...
    final tokenResponse = switch (oidcClientId) {
      ...
      final String oidcClientId =>
        await oidcRefresh(oidcClientId, refreshToken, resource: oidcResource),
    };
  ...
  Future<void> _innerSync({Duration? timeout}) async {
    ...
    try {
      ...
      Logs().w('Syncloop failed: Client has not connection to the server', e.originalException);
      onSyncStatus.add(
        SyncStatusUpdate(
          SyncStatus.error,
          error: SdkError(exception: e, stackTrace: s),
