...
extension Msc2964OidcLoginFlow on Client {
  ...
  /// `Client.oidcLogin()`.
  /// RFC 8707 resource indicators — when provided, the IDP will issue a JWT
  /// access_token with `aud` containing these resources instead of an opaque token.
  ...
  Future<void> oidcLogin({
    ...
    // RFC 8707: resource indicators for JWT access_token
    final resourceBody = <String, String>{};
    if (session.resource != null && session.resource!.isNotEmpty) {
      for (final r in session.resource!) {
        resourceBody['resource'] = r;
      }
    }
    final authMetadata = await getAuthMetadata();
    // When resource params present, encode manually to support multiple 'resource' values
    final String encodedBody;
    if (session.resource != null && session.resource!.isNotEmpty) {
      final parts = body.entries.map((e) =>
        '${Uri.encodeComponent(e.key)}=${Uri.encodeComponent(e.value)}');
      final resourceParts = session.resource!.map((r) =>
        'resource=${Uri.encodeComponent(r)}');
      encodedBody = [...parts, ...resourceParts].join('&');
    } else {
      encodedBody = body.entries.map((e) =>
  ...
  Future<OidcAuthResponse> oidcRefresh(
    ...
    if (resource != null && resource.isNotEmpty) {
      final parts = body.entries.map((e) =>
        '${Uri.encodeComponent(e.key)}=${Uri.encodeComponent(e.value)}');
      final resourceParts = resource.map((r) =>
        'resource=${Uri.encodeComponent(r)}');
      encodedBody = [...parts, ...resourceParts].join('&');
    } else {
      encodedBody = body.entries.map((e) =>
...
class OidcLoginSession {
  ...
  /// RFC 8707 resource indicators for JWT access_token audience
